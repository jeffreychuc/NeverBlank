...
class Tags extends React.Component  {
  constructor (props)  {
    ...
    this.renderTagAddModel = this.renderTagAddModel.bind(this);
  ...
  renderTagAddModal() {
    return (
      <div className = {!this.state.tagAddModal ? 'tag-add-modal active' : 'tag-add-modal'}>
        <div className='modalform tag-add-modal-form'>
    ...
    if (this.state.tagDeleteModal) {
      return (
        <div className='notebookDeleteModal'>
          ...
          <h2>Are you sure you want to delete <strong>{name}</strong>?</h2>
          <div className = 'notebookDeleteModalButtons'>
            <Button onClick={() => this.hideTagDeleteModal()}>Cancel</Button>
            <Button onClick={()=> this.handleTagDelete(id)} type="button">Delete</Button>
          </div>
        </div>
      );
    }
